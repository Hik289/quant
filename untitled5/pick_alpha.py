...
class pick_alpha(base_class.base_class):
    ...
    def alpha_indicator(self): #此函数功能为：1.算出alpha和风险因子的指标 2.根据风险因子的指标筛选alpha
        ...
        for i in range(0,len(self.alpha_list)):
            ...
            if config.is_test_predict_period: summary.to_csv(
                config.summary_path + '/summary_alpha%d.csv' % self.alpha_list[i])
        ...
        if config.is_test_predict_period: self.alpha_all_attributes_df.to_csv(
            config.summary_path + '/alpha_all_atrributes_table_%d_day.csv' % config.predict_period)
    ...
    def draw_picture(self):
        ...
        for i in range(0,len(self.alpha_list)):
            ...
            plotdata.plot()
            if config.is_test_predict_period: plt.savefig(
                config.draw_picture_path + '/alpha%d.png' % self.alpha_list[i])
    ...
    def select_alpha_correlation(self): #
        ...
        if len(good_alpha_f_df.columns) > 1: #如果只有一个alpha则不需要test correlation，只有多个alpha时才需要找相关性
            ...
            strong_corr_pairlist = base_class.base_class.findCorrelation(corr_matrix,
                                                                         config.alpha_correlation_threshold) # 调用base class中的函数
