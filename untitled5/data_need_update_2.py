...
# 处理历史数据,当不存在历史数据时，让它变成空的dataframe
def HistData(filename):
    try:
        hist_data = pd.read_csv(dir_path + '/' + filename, encoding='gbk', low_memory=False).set_index(
            'S_INFO_WINDCODE')
...
def getstock_amount(x, y):
    ...
    y_str = str(y)
    db = cx_Oracle.connect("wind_read_only", "wind_read_only", "192.168.0.223:1521/orcl")
    sql = "select s_info_windcode,trade_dt,s_dq_amount from wind.AShareEODPrices where trade_dt>=%s AND trade_dt<=%s " % (
    x_str, y_str)
    df = pd.read_sql(sql, db)
    d = pd.DataFrame(df)
    d = d.set_index(['TRADE_DT', 'S_INFO_WINDCODE'])
...
def getStock_Circulation_value(x, y):
    ...
    y_str = str(y)
    db = cx_Oracle.connect("wind_read_only", "wind_read_only", "192.168.0.223:1521/orcl")
    sql = "select s_info_windcode,trade_dt,s_dq_mv from wind.AShareEODDerivativeIndicator where trade_dt>=%s AND trade_dt<=%s " % (
    x_str, y_str)
    d = pd.read_sql(sql, db)
    d = d.set_index(['TRADE_DT', 'S_INFO_WINDCODE'])
    d = d.stack(level=0)
...
def get_operation_revenue(x, y):
    ...
    y_str = str(y)
    db = cx_Oracle.connect("wind_read_only", "wind_read_only", "192.168.0.223:1521/orcl")
    sql = "select s_info_windcode,report_period,tot_oper_rev from wind.AShareIncome where statement_type=408001000 AND report_period>=%s AND report_period<=%s order by report_period " % (
    x_str, y_str)
    df = pd.read_sql(sql, db)
    d = pd.DataFrame(df)
    if d.empty != True:
...
def get_operation_revenue_ss(x, y):
    ...
    y_str = str(y)
    db = cx_Oracle.connect("wind_read_only", "wind_read_only", "192.168.0.223:1521/orcl")
    sql = "select s_info_windcode,report_period,ann_dt,tot_oper_rev from wind.AShareIncome where statement_type=408002000 AND report_period>=%s AND report_period<=%s order by report_period " % (
    x_str, y_str)
    df = pd.read_sql(sql, db)
    if df.empty != True:
        gor = pd.DataFrame(df)
...
def get_eps_est(x, y):
    ...
    y_str = str(y)
    db = cx_Oracle.connect("wind_read_only", "wind_read_only", "192.168.0.223:1521/orcl")
    sql = "select s_info_windcode,est_dt,est_report_dt,eps_avg from wind.AShareConsensusData where est_dt>=%s AND est_dt<=%s order by s_info_windcode" % (
    x_str, y_str)
    df = pd.read_sql(sql, db)
    d = pd.DataFrame(df)
...
def get_flag():  # 把不可用的数据点的位置设置为NAN，然后乘以return的矩阵，达到剔除的效果（代码可精简）
    ...
    # 有可能开始/结束读取数据的日期不是交易日，更精确一步获取开始与结束的日子
    start_dt = timelist[0]
    end_dt = timelist[-1]
...
if is_trade_date(enddate) == 0:
    ...
    try:
        ...
        if is_trade_date(startdate) == 0:
            ...
            startdate = startdate
        getStock_status(startdate, enddate)
    stock_status = pd.read_csv(dir_path + '/stock_status.csv', encoding='gbk', low_memory=False).set_index(
        'S_INFO_WINDCODE')
