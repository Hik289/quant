...
des_path='/usr/intern/zztest/risk_factors'
dir_rfhist_path='/usr/intern/zztest/risk_factors'
...
def get_momentum():
    ...
    new_mmt = new_mmt.iloc[:,startindex:endindex+1]
    #
    #
    # weight = []  # 长度为500
...
def get_liquidity():#应该需要重写，T1求和时应为3*21，T2求和应为21*12
    ...
    liquidity = liquidity.loc[:,startdate:enddate] #只取更新的片段
...
def get_growth_factor(factor_name):
    ...
    drop_duplicated = data.duplicated(["S_INFO_WINDCODE","REPORT_PERIOD"], keep="last")  # 产生布尔值，删除值 = True
    data = data[drop_duplicated == False]
...
def get_growth():#其他3个算出来，加上权重,
    ...
    #若空，权重调为0,其他重新单位化为1
    SGRO = get_growth_factor("SGRO").loc[:,:enddate]
    EGRO = get_growth_factor("EGRO").loc[:,:enddate]
    EGIB = get_EGIB().loc[:,:enddate]
    EGIB_s = get_EGIB_s().loc[:,:enddate]
    ...
    growth = growth.loc[:,startdate:enddate]
...
def get_beta_hsigma(): #原意应为对X和Y的序列按250天分别加权平均，再进行OLS，或直接用公式计算
    """
    求beta值：所需要的数据至少为 2 * window_length
    :return:返回BARRA HISGMA值
    """
    hs300 = (pd.read_csv(dir_path + '/hs300_return.csv', index_col=0))/100  #百分制收益率
    window_length = 250
    data_window = window_length * 3           # 采用window长度的三倍作为数据window
    def ewma(window):
        window_series = pd.Series(window)
        ewma_result = window_series.ewm(halflife=60).mean()
        return ewma_result[window_length-1]
   
    Return_part = Return.iloc[:,startindex-data_window:endindex+1]
...
#    beta_df_ewma = beta_df.rolling(window=window_length, axis=1).apply(ewma)
    ...
    beta_df_ewma = beta_df.rolling(window=window_length,min_periods=200, axis=1).apply(ewma,raw = True)
...
#小类因子量纲一值，逻辑相似，小类因子在此先不标准化，最后合成大类因子后再标准化
    ...
    volatility = volatility.loc[:,startdate:enddate]
